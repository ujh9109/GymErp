 ...
 * ------------------------------------------------------------
 * PT(개인 트레이닝) 예약 관련 로직을 담당하는 서비스 클래스.
 * REGISTRATION 테이블의 CRUD를 수행하며,
 * 로그(PT_LOG)는 ScheduleServiceImpl에서 처리한다.
 *
 * 주요 기능:
 *  - PT 예약 등록/수정/삭제
 *  - 일정번호(shNum)로 등록번호(regNum) 조회
 */
@Service
...
public class PtRegistrationServiceImpl implements PtRegistrationService {
     ...
     * 전체 PT 예약 조회
     * ------------------------------------------------------------
     * 현재는 empNum, date 파라미터를 사용하지 않지만,
     * 향후 트레이너별 / 날짜별 필터 기능 추가를 고려한 구조이다.
     */
    @Override
     ...
     * PT 예약 등록
     * ------------------------------------------------------------
     * 1️⃣ Mapper를 통해 REGISTRATION 테이블에 데이터 삽입
     ...
     * PT 예약 삭제
     * ------------------------------------------------------------
     * 1️⃣ 삭제 전 regNum으로 해당 예약 존재 여부 확인
     * 2️⃣ 존재하지 않으면 예외 발생
     ...
     * ------------------------------------------------------------
     * SCHEDULE 삭제 시 REGISTRATION의 regNum을 찾기 위해 사용.
     * ex) 일정 삭제 시 → 해당 일정(shNum)에 연결된 REGISTRATION 찾기
     */
    @Override
